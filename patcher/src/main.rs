...
fn create_framework_map() {
    ...
    for captures in regex.captures_iter(&content) {
        let mangled = captures.get(1).unwrap().as_str().trim();
        let mangled = end_removal
            .captures(mangled)
            .map_or(mangled, |c| c.get(1).unwrap().as_str());
        ...
        let length = captures.get(3).unwrap().as_str();
        let source_file = captures.get(4).unwrap().as_str();
        let length = u32::from_str_radix(length, 16).unwrap();

        let mut fn_name: &str = &fn_name
            .replace(' ', "_")
            .replace("()", "Void")
            .replace("(", "Tuple<")
            .replace(")", ">");
        ...
        if address != 0 {
            writeln!(
                file,
                "  00000000 {:06x} {:08x}  4 {} \t{}",
                length, address, fn_name, source_file
            ).unwrap();
        }
    }
...
fn main() {
    ...
    let _ = File::open("../../src/patch.asm")
        .expect(
            "Couldn't find \"src/patch.asm\". If you don't need to patch the dol, just \
    ...
    let _ = File::open("../../target/intermediate.dol")
        .expect(
            "Couldn't find \"target/intermediate.dol\". Did you build the project correctly \
             using \"make\"?",
        )
    ...
    if let Some("cheat") = args().skip(1).next().as_ref().map(|x| x as &str) {
        ...
        let _ = File::open("../../game/original.dol")
            .expect(
                "Couldn't find \"game/original.dol\". You need to copy the game's main.dol \
                 there.",
            )
...
fn patch_game(original: DolFile, intermediate: DolFile, instructions: &[Instruction]) {
    ...
    let mut file = File::create("../../game/sys/main.dol").expect(
        "Couldn't create \"game/sys/main.dol\". You might need to provide higher \
         privileges.",
    );
